TITLE Mod file for component: Component(id=nax__sh15 type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=nax__sh15 type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gatesHHrates[*]/fcond, on: Component(id=nax__sh15 type=ionChannelHH), from gatesHHrates; null
    ? Path not present in component, using factor: 1
    
    fopenHHrates = 1 
    
    ? DerivedVariable is based on path: gatesHHtauInf[*]/fcond, on: Component(id=nax__sh15 type=ionChannelHH), from gatesHHtauInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHtauInf = 1 
    
    ? DerivedVariable is based on path: gatesHHratesTau[*]/fcond, on: Component(id=nax__sh15 type=ionChannelHH), from gatesHHratesTau; Component(id=m type=gateHHratesTau)
    fopenHHratesTau = m_fcond ? multiply applied to all instances of fcond in: <gatesHHratesTau> ([Component(id=m type=gateHHratesTau)]) c2 ([Component(id=null type=annotation), Component(id=null type=notes), Component(id=h type=gateHHratesTauInf), Component(id=m type=gateHHratesTau)]) ? path based
    
    ? DerivedVariable is based on path: gatesHHratesInf[*]/fcond, on: Component(id=nax__sh15 type=ionChannelHH), from gatesHHratesInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesInf = 1 
    
    ? DerivedVariable is based on path: gatesHHratesTauInf[*]/fcond, on: Component(id=nax__sh15 type=ionChannelHH), from gatesHHratesTauInf; Component(id=h type=gateHHratesTauInf)
    fopenHHratesTauInf = h_fcond ? multiply applied to all instances of fcond in: <gatesHHratesTauInf> ([Component(id=h type=gateHHratesTauInf)]) c2 ([Component(id=null type=annotation), Component(id=null type=notes), Component(id=h type=gateHHratesTauInf), Component(id=m type=gateHHratesTau)]) ? path based
...
PROCEDURE rates() {
    ...
    h_rateScale = h_q10Settings_q10 ? multiply applied to all instances of q10 in: <q10Settings> ([Component(id=null type=q10ExpTemp)]) c2 ([Component(id=null type=nax__sh15_h_inf_inf), Component(id=null type=nax__sh15_h_alpha_rate), Component(id=null type=nax__sh15_h_beta_rate), Component(id=null type=nax__sh15_h_tau_tau), Component(id=null type=q10ExpTemp)]) ? path based
    
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=h type=gateHHratesTauInf), from forwardRate; Component(id=null type=nax__sh15_h_alpha_rate)
    h_alpha = h_forwardRate_r ? path based
    
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=h type=gateHHratesTauInf), from reverseRate; Component(id=null type=nax__sh15_h_beta_rate)
    h_beta = h_reverseRate_r ? path based
    
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=h type=gateHHratesTauInf), from steadyState; Component(id=null type=nax__sh15_h_inf_inf)
    h_inf = h_steadyState_x ? path based
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHratesTauInf), from timeCourse; Component(id=null type=nax__sh15_h_tau_tau)
    ...
    m_rateScale = m_q10Settings_q10 ? multiply applied to all instances of q10 in: <q10Settings> ([Component(id=null type=q10ExpTemp)]) c2 ([Component(id=null type=nax__sh15_m_alpha_rate), Component(id=null type=nax__sh15_m_beta_rate), Component(id=null type=nax__sh15_m_tau_tau), Component(id=null type=q10ExpTemp)]) ? path based
    
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=m type=gateHHratesTau), from forwardRate; Component(id=null type=nax__sh15_m_alpha_rate)
    m_alpha = m_forwardRate_r ? path based
    
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=m type=gateHHratesTau), from reverseRate; Component(id=null type=nax__sh15_m_beta_rate)
    m_beta = m_reverseRate_r ? path based
    
    m_fcond = m_q ^ m_instances ? evaluable
    m_inf = m_alpha /( m_alpha + m_beta ) ? evaluable
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHratesTau), from timeCourse; Component(id=null type=nax__sh15_m_tau_tau)
